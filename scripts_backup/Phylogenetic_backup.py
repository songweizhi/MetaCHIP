...
parser.add_argument('-t', required=False, default=None, help='species (SCG) tree')
...
parser.add_argument('-flk', required=False, type=int, default=10, help='the length of flanking sequences to plot (Kbp)')
...
log_file_name =          '%s_PG_%s.log'   % (output_prefix, datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
...
MetaCHIP_op_folder = '%s_HGTs_ip%s_al%sbp_c%s_e%sbp_f%skbp_%s%s' % (output_prefix, str(identity_percentile), str(align_len_cutoff), str(cover_cutoff), str(at_ends_length_cutoff), flanking_length_kbp, grouping_rank, get_number_of_group(pwd_grouping_file))
...
at_ends_folder_name =                               '2_Plots_at_ends'
at_ends_folder_name_PG_validated =                  '2_Plots_at_ends_PG_validated'
full_contig_match_folder_name =                     '3_Plots_contig_match'
full_contig_match_folder_name_PG_validated =        '3_Plots_contig_match_PG_validated'
at_ends_and_contig_match_folder_name =              '4_Plots_at_ends_and_contig_match'
at_ends_and_contig_match_folder_name_PG_validated = '4_Plots_at_ends_and_contig_match_PG_validated'
...
plot_identity_distribution_BM =                     '%s_plot_HGT_identities_BM.png'   % output_prefix
plot_identity_distribution_PG =                     '%s_plot_HGT_identities_PG.png'   % output_prefix
plot_location_stats =                               '%s_plot_location_stat.png'       % output_prefix
...
pwd_plot_location_stats =                           '%s/%s'                           % (pwd_MetaCHIP_op_folder, plot_location_stats)
...
pwd_4_at_ends_and_contig_match_folder =             '%s/%s'                           % (pwd_flanking_region_plot_folder, at_ends_and_contig_match_folder_name)
pwd_4_at_ends_and_contig_match_folder_PG_validated ='%s/%s'                           % (pwd_flanking_region_plot_folder, at_ends_and_contig_match_folder_name_PG_validated)
...
# if os.path.isdir(pwd_ranger_wd):
...
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'store ortholog information into dictionary\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'store ortholog information into dictionary')
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'get species tree subset for each ortholog\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'get species tree subset for each ortholog')


def subset_species_tree_worker(argument_list):
    each_candidates = argument_list[0]
    clusters_dict = argument_list[1]
...
#     list_for_multiple_arguments_subset_species_tree.append([each_candidate, clusters_dict, pwd_tree_folder])
#
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Get dicts\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Get dicts')
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Prepare subset of %s\n' % combined_faa_file)
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Prepare subset of %s' % combined_faa_file)
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'The number of gene sequences to extract: %s\n' % len(gene_id_uniq_set))
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'The number of gene sequences to extract: %s' % len(gene_id_uniq_set))
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Extract sequences for building gene tree with %s cores\n' % num_threads)
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Extract sequences for building gene tree with %s cores' % num_threads)

# # put multiple arguments in list
# list_for_multiple_arguments_extract_gene_tree_seq = []
# for each_to_extract in candidates_list_for_Phylogenetic:
#     list_for_multiple_arguments_extract_gene_tree_seq.append([each_to_extract, candidates_gene_id_dict, pwd_tree_folder, pwd_combined_faa_file_subset, pwd_blastp_exe, pwd_mafft_exe, pwd_fasttree_exe])
#
# pool = mp.Pool(processes=num_threads)
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Running Ranger-DTL-U\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Running Ranger-DTL-U')
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Parsing Ranger prediction results\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Parsing Ranger prediction results')
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime('[%Y-%m-%d %H:%M:%S]') + ' Add Ranger-DTL predicted direction to HGT_candidates.txt\n')
if keep_quiet == 0:
...
combined_output_validated_header = 'Gene_1\tGene_2\tGene_1_group\tGene_2_group\tIdentity\tAt_ends\tCtg_align\tDirection\n'
...
for match_group in open(pwd_candidates_file):
    if not match_group.startswith('Gene_1'):
        ...
        #if (end_break == 'no') and (validated_prediction != 'NA'):
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Done for Phylogenetic approach!\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Done for Phylogenetic approach!')


###################################### separate PG validated flanking region plots #####################################

# os.mkdir(pwd_1_normal_folder_PG_validated)
# os.mkdir(pwd_2_at_ends_folder_PG_validated)
# os.mkdir(pwd_3_full_contig_match_folder_PG_validated)
# os.mkdir(pwd_4_at_ends_and_contig_match_folder_PG_validated)
...
for PG_HGT in open(pwd_candidates_file_ET):
    if PG_HGT != combined_output_validated_header:
        ...
        if (At_ends == 'no') and (Ctg_align == 'no') and (Direction != 'NA'):
            ...
            if os.path.isfile(pwd_possible_file_1) == True:
                os.system('cp %s %s/' % (pwd_possible_file_1, pwd_1_normal_folder_PG_validated))
            if os.path.isfile(pwd_possible_file_2) == True:
                os.system('cp %s %s/' % (pwd_possible_file_2, pwd_1_normal_folder_PG_validated))
        ...
        if (At_ends == 'yes') and (Ctg_align == 'no') and (Direction != 'NA'):
            ...
            if os.path.isfile(pwd_possible_file_1) == True:
                os.system('cp %s %s/' % (pwd_possible_file_1, pwd_2_at_ends_folder_PG_validated))
            if os.path.isfile(pwd_possible_file_2) == True:
                os.system('cp %s %s/' % (pwd_possible_file_2, pwd_2_at_ends_folder_PG_validated))
        ...
        if (At_ends == 'no') and (Ctg_align == 'yes') and (Direction != 'NA'):
            ...
            if os.path.isfile(pwd_possible_file_1) == True:
                os.system('cp %s %s/' % (pwd_possible_file_1, pwd_3_full_contig_match_folder_PG_validated))
            if os.path.isfile(pwd_possible_file_2) == True:
                os.system('cp %s %s/' % (pwd_possible_file_2, pwd_3_full_contig_match_folder_PG_validated))

        # at ends and contig match
        if (At_ends == 'yes') and (Ctg_align == 'yes') and (Direction != 'NA'):
            pwd_possible_file_1 = '%s/%s' % (pwd_4_at_ends_and_contig_match_folder, possible_file_name_1)
            pwd_possible_file_2 = '%s/%s' % (pwd_4_at_ends_and_contig_match_folder, possible_file_name_2)
            if os.path.isfile(pwd_possible_file_1) == True:
                os.system('cp %s %s/' % (pwd_possible_file_1, pwd_4_at_ends_and_contig_match_folder_PG_validated))
            if os.path.isfile(pwd_possible_file_2) == True:
                os.system('cp %s %s/' % (pwd_possible_file_2, pwd_4_at_ends_and_contig_match_folder_PG_validated))
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Plot stats of identified HGTs\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Plot stats of identified HGTs')
...
HGT_num_BM_at_end_and_ctg_aln = 0
HGT_num_PG_normal = 0
HGT_num_PG_at_end = 0
HGT_num_PG_ctg_aln = 0
HGT_num_PG_at_end_and_ctg_aln = 0
identity_list_BM_normal = []
identity_list_BM_at_end = []
identity_list_BM_ctg_aln = []
identity_list_BM_at_end_and_ctg_aln = []
identity_list_PG_normal = []
identity_list_PG_at_end = []
identity_list_PG_ctg_aln = []
identity_list_PG_at_end_and_ctg_aln = []
for each_HGT in open(pwd_candidates_file_ET):
    if not each_HGT.startswith('Gene_1'):
        ...
        Ctg_align = each_HGT_split[6]
        PG_validation = each_HGT_split[7]

        # get number for normal
        if (at_end == 'no') and (Ctg_align == 'no'):
            HGT_num_BM_normal += 1
        ...
        if (at_end == 'yes') and (Ctg_align == 'no'):
            HGT_num_BM_at_end += 1
            identity_list_BM_at_end.append(identity)
            if PG_validation != 'NA':
                HGT_num_PG_at_end += 1
                identity_list_PG_at_end.append(identity)

        # get number for Ctg_align
        if (at_end == 'no') and (Ctg_align == 'yes'):
            HGT_num_BM_ctg_aln += 1
            identity_list_BM_ctg_aln.append(identity)
            if PG_validation != 'NA':
                HGT_num_PG_ctg_aln += 1
                identity_list_PG_ctg_aln.append(identity)

        # get number for at_end and Ctg_align
        if (at_end == 'yes') and (Ctg_align == 'yes'):
            HGT_num_BM_at_end_and_ctg_aln += 1
            identity_list_BM_at_end_and_ctg_aln.append(identity)
            if PG_validation != 'NA':
                HGT_num_PG_at_end_and_ctg_aln += 1
                identity_list_PG_at_end_and_ctg_aln.append(identity)
...
at_end_and_ctg_match_list = (HGT_num_BM_at_end_and_ctg_aln, HGT_num_PG_at_end_and_ctg_aln)
...
rects1 = plt.bar(index + bar_width*0, normal_list, bar_width, alpha=0.4, color='g', label='Normal', align='center')
rects2 = plt.bar(index + bar_width*1, at_end_list, bar_width, alpha=0.4, color='b', label='At Ends', align='center')
rects3 = plt.bar(index + bar_width*2, ctg_match_list, bar_width, alpha=0.4, color='orange', label='Ctg Matched', align='center')
rects4 = plt.bar(index + bar_width*3, at_end_and_ctg_match_list, bar_width, alpha=0.4, color='r', label='AE and CM', align='center')
...
plt.tight_layout()
plt.savefig(pwd_plot_location_stats, dpi=300)
plt.close()

# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Location stats exported to %s\n' % (plot_location_stats))
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Location stats exported to %s' % (plot_location_stats))
...
##### for BM approach #####
num_bins = 50
combined_list_BM = (identity_list_BM_normal, identity_list_BM_at_end, identity_list_BM_at_end_and_ctg_aln, identity_list_BM_ctg_aln)
color_list = ['g', 'b', 'r', 'orange']
label_list = ['Normal', 'At Ends', 'AE_and_CM', 'Ctg Matched']
plt.hist(combined_list_BM, num_bins, alpha=0.6, normed=0, stacked=1, linewidth=0, color=color_list, label=label_list, rwidth=0.8)
lgd = plt.legend(prop={'size': 10}, ncol=1, bbox_to_anchor=(1.28, 1))
total_HGT_num_BM = len(identity_list_BM_normal) + len(identity_list_BM_at_end) + len(identity_list_BM_at_end_and_ctg_aln) + len(identity_list_BM_ctg_aln)
plt.title('Identity distribution of %s BM identified HGTs' % total_HGT_num_BM)
plt.xlabel('Identity (%)')
...
# Get plot
plt.tight_layout()
plt.savefig(pwd_plot_identity_distribution_BM, bbox_extra_artists=(lgd,), bbox_inches='tight', dpi=300)
plt.close()
plt.clf()
...
##### for PG approach #####
num_bins = 50
combined_list_PG = (identity_list_PG_normal, identity_list_PG_at_end, identity_list_PG_at_end_and_ctg_aln, identity_list_PG_ctg_aln)
color_list = ['g', 'b', 'r', 'orange']
label_list = ['Normal', 'At Ends', 'AE_and_CM', 'Ctg Matched']
plt.hist(combined_list_PG, num_bins, alpha=0.6, normed=0, stacked=1, linewidth=0, color=color_list, label=label_list, rwidth=0.8)
lgd = plt.legend(prop={'size': 10}, ncol=1, bbox_to_anchor=(1.28, 1))
total_HGT_num_PG = len(identity_list_PG_normal) + len(identity_list_PG_at_end) + len(identity_list_PG_at_end_and_ctg_aln) + len(identity_list_PG_ctg_aln)
plt.title('Identity distribution of %s PG validated HGTs' % total_HGT_num_PG)
plt.xlabel('Identity (%)')
plt.ylabel('Number of HGT')
plt.subplots_adjust(left=0.15)
...
# Get plot
plt.tight_layout()
plt.savefig(pwd_plot_identity_distribution_PG, bbox_extra_artists=(lgd,), bbox_inches='tight', dpi=300)
plt.close()
plt.clf()

# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Identity distribution of identified HGTs exported to %s and %s\n' % (plot_identity_distribution_BM, plot_identity_distribution_PG))
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Identity distribution of identified HGTs exported to %s and %s' % (plot_identity_distribution_BM, plot_identity_distribution_PG))
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Gene flow plot exported to %s\n' % plot_circos)
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Gene flow plot exported to %s' % plot_circos)
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'All done!\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'All done!')
