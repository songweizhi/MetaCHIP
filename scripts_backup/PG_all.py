...
def extract_gene_tree_seq_worker(argument_list):
    ...
    cmd_mafft = '%s --quiet --maxiterate 1000 --globalpair %s > %s' % (
    pwd_mafft_exe, gene_tree_seq_uniq, pwd_seq_file_1st_aln)
...
def PG(args):
    ...
    MetaCHIP_op_folder = '%s_%s%s_ip%s_al%sbp_c%s_ei%sbp_f%skbp' % (output_prefix, grouping_level, group_num, str(identity_percentile), str(align_len_cutoff), str(cover_cutoff), str(end_match_identity_cutoff), flanking_length_kbp)
    ...
    tree_folder =                                       '%s_%s%s_tree_folder'                         % (output_prefix, grouping_level, group_num)
    ranger_inputs_folder_name =                         '%s_%s%s_Ranger_input'                        % (output_prefix, grouping_level, group_num)
    ranger_outputs_folder_name =                        '%s_%s%s_Ranger_output'                       % (output_prefix, grouping_level, group_num)
    ...
    ranger_wd_name =                                    '%s_%s%s_Ranger-DTL_wd'                       % (output_prefix, grouping_level, group_num)
    output_tree_folder_name =                           '%s_%s%s_PG_wd'                               % (output_prefix, grouping_level, group_num)
    ...
    # for report and log
    report_and_log(('store ortholog information into dictionary'), pwd_log_file, keep_quiet)
    ...
    # for report and log
    report_and_log(('get species tree subset for each ortholog'), pwd_log_file, keep_quiet)

    # put multiple arguments in list
    list_for_multiple_arguments_subset_species_tree = []
    for each_candidate in candidates_list:
        list_for_multiple_arguments_subset_species_tree.append([each_candidate, pwd_newick_tree_file, clusters_dict, pwd_tree_folder])
    ...
    # for report and log
    report_and_log(('Running mafft and fasttree with %s cores' % num_threads), pwd_log_file, keep_quiet)
