...
soil_hgts = set()
hgts_from_soil_genome = set()
hgts_to_soil_genome = set()
for each_hgt in open(MetaCHIP_hgts):
    ...
    if not each_hgt.startswith('Gene_1\tGene_2'):
        ...
        if identity >= 99:
            ...
            if donor_genome in soil_genome_id_list:
                if gene_1_genome == donor_genome:
                    hgts_from_soil_genome.add(gene_1)
                else:
                    hgts_from_soil_genome.add(gene_2)

            if recipient_genome in soil_genome_id_list:
                if gene_1_genome == recipient_genome:
                    hgts_to_soil_genome.add(gene_1)
                else:
                    hgts_to_soil_genome.add(gene_2)
...
print('The number of recent HGTs transferred from soil genome: %s' % len(hgts_from_soil_genome))
print('The number of recent HGTs transferred to soil genome: %s' % len(hgts_to_soil_genome))


MetaCHIP_hgts_aa_seq_soil = '/Users/songweizhi/Desktop/Total2094_g664_HGTs_PG_aa_to_soil298.fasta'
MetaCHIP_hgts_aa_seq_soil_handle = open(MetaCHIP_hgts_aa_seq_soil, 'w')
for each_aa in SeqIO.parse(MetaCHIP_hgts_aa_seq, 'fasta'):
    if str(each_aa.id) in hgts_to_soil_genome:
...
cog_stats_file = '/Users/songweizhi/Desktop/to_soil298_cog_stats.txt'
...
protein_id_cog_file = '/Users/songweizhi/Desktop/to_soil298_protein-id_cog.txt'
...
for each_hgt_gene in hgts_to_soil_genome:
