...
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Input genomes were clustered into %s groups.\n' % group_num)
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Input genomes were clustered into %s groups. ' % group_num)

if group_num == 1:
    sleep(0.5)
    print(datetime.now().strftime(time_format) + 'Group number is too low for HGT analysis, please provide a lower rank level')
    exit()

# report ignored genomes
sleep(0.5)
print(datetime.now().strftime(time_format) + 'The number of genome with unknown classification at specified level is: %s.' % ignored_genome_num)
if ignored_genome_num > 0:
    sleep(0.5)
    print(datetime.now().strftime(time_format) + 'These genomes will be excluded for further HGT analysis. You might want to try with a higher rank level if this number is too high.')
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Grouping file exported to: %s.\n' % grouping_file_name)
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Grouping file exported to: %s.' % grouping_file_name)
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Plot grouping stat\n')
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Plot grouping stat')
...
# for report and log
with open(pwd_log_file_name, 'a') as log_handle:
    log_handle.write(datetime.now().strftime(time_format) + 'Grouping stats exported to: %s.\n' % grouping_plot_name)
if keep_quiet == 0:
    print(datetime.now().strftime(time_format) + 'Grouping stats exported to: %s.' % grouping_plot_name)
...
if grouping_only == False:
    ...
    # for report and log
    with open(pwd_log_file_name, 'a') as log_handle:
        log_handle.write(datetime.now().strftime(time_format) + 'The size of input genomes exported to: %s.\n' % genome_size_file_name)
    if keep_quiet == 0:
        print(datetime.now().strftime(time_format) + 'The size of input genomes exported to: %s.' % genome_size_file_name)
...
if grouping_only == False:
    ...
    if noblast is True:
        ...
        # for report
        with open(pwd_log_file_name, 'a') as log_handle:
            log_handle.write(datetime.now().strftime(time_format) + 'All-vs-all blastn disabled, please run it with parameters specified as below:\n')
            log_handle.write(datetime.now().strftime(time_format) + '%s\n' % all_vs_all_blast_cmd)
        if keep_quiet == 0:
            print(datetime.now().strftime(time_format) + 'All-vs-all blastn disabled, please run it with parameters specified as below:')
            print(datetime.now().strftime(time_format) + '%s' % all_vs_all_blast_cmd)
