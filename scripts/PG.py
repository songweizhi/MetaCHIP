...
def Ranger_worker(argument_list):
        ...
        # change species tree leaf name for Ranger-DTL2, replace "_" with "XXXXX"
        for each_gr_leaf in species_tree:
            each_gr_leaf.name = 'XXXXX'.join(each_gr_leaf.name.split('_'))

        # change gene tree leaf name for Ranger-DTL2, replace "_" with "XXXXX"
        for each_gr_leaf in gene_tree:
            #each_gr_leaf.name = '_'.join(each_gr_leaf.name.split('_')[:-1])
            # each_gr_leaf.name = '%s_%s' % (('#'.join(each_gr_leaf.name.split('_')[:-1])), (each_gr_leaf.name.split('_')[-1]))
            each_gr_leaf.name = 'XXXXX'.join(each_gr_leaf.name.split('_')[:-1])
...
def PG(args):
    ...
    # for report and log
    report_and_log(('Running Ranger-DTL-U'), pwd_log_file_name, keep_quiet)
