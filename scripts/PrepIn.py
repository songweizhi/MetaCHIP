...
def mafft_worker(argument_list):
    ...
    mafft_cmd = '%s --quiet --maxiterate 1000 --globalpair %s > %s ; rm %s' % (
    pwd_mafft_exe, fastaFile1, fastaFile2, fastaFile1)
    os.system(mafft_cmd)
...
def PrepIn(args):
    ...
    input_genome_file_list_no_ext = ['_'.join(i.split('.')[:-1]) for i in input_genome_file_list]
    ...
    if grouping_only is True:
        report_and_log('running with grouping-only mode', pwd_log_file_name, keep_quiet)
    else:
    ...
    if input_genome_file_list == []:
        report_and_log('No input genome detected, program exited!', pwd_log_file_name, keep_quiet)
        exit()
    ...
    for each_genome in open(GTDB_output_file):
        ...
        if bin_name in input_genome_file_list_no_ext:
    ...
    # for report and log
    report_and_log(('Input genomes clustered into %s groups' % group_num), pwd_log_file_name, keep_quiet)

    if group_num == 1:
        sleep(0.5)
        report_and_log('Group number is too low for HGT analysis, please provide a lower rank level', pwd_log_file_name, keep_quiet)
    ...
    if ignored_genome_num > 0:
        report_and_log(('The number of genome with unknown classification at specified level is: %s' % ignored_genome_num), pwd_log_file_name, keep_quiet)
        report_and_log('These genomes will be excluded for further HGT analysis. You might want to try with a higher taxonomic rank if the number is too high', pwd_log_file_name, keep_quiet)
    ...
    sleep(0.5)
    # for report and log
    report_and_log(('Grouping file exported to: %s' % grouping_file_name), pwd_log_file_name, keep_quiet)
    ...
    plt.bar(x_range,
            group_id_uniq_count,
            tick_label=group_id_with_taxon,
            align='center',
            alpha=0.2)
    ...
    # for report and log
    report_and_log(('Grouping stats exported to: %s' % grouping_plot_name), pwd_log_file_name, keep_quiet)
    ...
    if grouping_only == False:
        ...
        for each_genome in input_genome_file_list:
        ...
        # for report and log
        report_and_log(('The size of input genomes exported to: %s' % genome_size_file_name), pwd_log_file_name, keep_quiet)
    ...
    if grouping_only == False:
        ...
        # for report and log
        report_and_log(('Running Prodigal with %s cores' % num_threads), pwd_log_file_name, keep_quiet)
        ...
        ################################### define file name and create working directory ##################################
        ...
        # for report and log
        report_and_log(('Running hmmsearch with %s cores' % num_threads), pwd_log_file_name, keep_quiet)
        ...
        # for report and log
        report_and_log(('Running mafft with %s cores' % num_threads), pwd_log_file_name, keep_quiet)
        ...
        # for report and log
        report_and_log('Concatenating alignments', pwd_log_file_name, keep_quiet)
        ...
        # for report and log
        report_and_log('Running fasttree', pwd_log_file_name, keep_quiet)
        ...
        # for report and log
        report_and_log(('Species tree exported to: %s' % newick_tree_file), pwd_log_file_name, keep_quiet)

        ############################################### run all vs all blastn ##############################################

        # for report and log
        report_and_log(('Running all-vs-all blastn for %s with the following command, be patient...' % combined_ffn_file), pwd_log_file_name, keep_quiet)
        ...
        if noblast is True:
            report_and_log('All-vs-all blastn disabled, please run it with parameters specified as below:', pwd_log_file_name, keep_quiet)
            report_and_log(all_vs_all_blast_cmd, pwd_log_file_name, keep_quiet)
        ...
        report_and_log('All protein orthologs within the input genomes need to be obtained for PG approach, you can get them with the following commands:', pwd_log_file_name, keep_quiet)
        report_and_log(('$ get_homologues.pl -f 70 -t 3 -S 70 -E 1e-05 -C 70 -G -n 16 -d %s -X' % gbk_folder), pwd_log_file_name, keep_quiet)
