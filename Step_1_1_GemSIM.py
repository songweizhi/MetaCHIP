...
GemReads_parameters = '-q 33 -u d -s 30 -n 10000 -l d -p'
...
if os.path.isdir(pwd_GemSIM_wd):
    ...
    os.mkdir('%s/qsub_files' % pwd_GemSIM_wd)
else:
    os.mkdir(pwd_GemSIM_wd)
    os.mkdir('%s/qsub_files' % pwd_GemSIM_wd)
...
while n <= number_of_replicates:
    ...
    pwd_current_qsub_file = '%s/qsub_files/qsub_GemSIM_abundance_%s.sh' % (pwd_GemSIM_wd, n)
